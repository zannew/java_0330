...
import phoneNumBook_ver_6_2_exception_submit.StringEmptyException;
...
public class PhonebookManager {
	...
	private PhoneInfo_abs info;			//PhoneInfo클래스 변수선언
	private ArrayList<PhoneInfo_abs> pBook;


	//생성자 초기화
	public PhonebookManager(int numOfFriend) {
		//인스턴스(제네릭클래스)를 PhoneInfo타입 데이터만 저장할 수 있도록 생성
		pBook=new ArrayList<PhoneInfo_abs>();
	}

	//정보입력하고 객체 생성
	PhoneInfo_abs createInst(int selectOpt2) {
	...
	void storeInst(PhoneInfo_abs info) {
	...
	void editInfo() {
		...
		if(searchIndex<0) {
			...
			}else if(pBook.get(searchIndex) instanceof PhoneInfo_abs) {
