...
select e.job from emp e where e.empno=7788;
...
select rownum, job  
from (select e.job, avg(e.sal) from emp e group by job) 
where rownum=1
;

select rownum, e.job, avg(sal)
from emp e
where sal in (select avg(m.sal), m.job from emp m group by m.job) and rownum=1
;
...
select *
from emp
where sal > (select avg(sal) from emp where deptno = (select deptno from emp e where e.ename like '%M%'))
;
